...
export const migrateUserNames = internalAction({
  args: {},
  handler: async (ctx): Promise<{totalUsers: number, successCount: number, errorCount: number, message: string, note: string}> => {
    // Get users who are missing firstName and lastName AND have been active in the last 24 hours
    // Facebook only allows profile queries for recently active users
    const usersWithoutNames: Doc<"users">[] = await ctx.runQuery(internal.users.getRecentlyActiveUsersWithoutNames);

    console.log(`Found ${usersWithoutNames.length} recently active users without names to migrate`);
    ...
    if (usersWithoutNames.length === 0) {
      return {
        ...
        successCount: 0,
        errorCount: 0,
        message: "No recently active users found without names.",
        note: "Facebook Messenger API only allows profile queries during active conversations. Names will be collected organically when users interact with the bot."
      };
    }
    ...
    for (const user of usersWithoutNames) {
      try {
        ...
        if (!accessToken) {
          console.error("ACCESS_TOKEN not configured for migration");
          errorCount++;
          continue;
        }
        ...
        if (userProfile?.first_name || userProfile?.last_name) {
          ...
          successCount++;
          console.log(`Migrated user ${user.messengerId}: ${userProfile?.first_name} ${userProfile?.last_name}`);
        } else {
          console.log(`No name data available for user ${user.messengerId} (may be privacy restricted or PSID expired)`);
        }
      } catch (error) {
    ...
    return {
      totalUsers: usersWithoutNames.length,
      successCount,
      errorCount,
      message: `Migration completed. ${successCount} users updated, ${errorCount} errors.`,
      note: "Facebook Messenger API restrictions: Profile data can only be accessed during active conversations. Remaining users will get their names collected organically during future interactions."
    };
  },
});
...
export const getUsersWithoutNames = internalQuery({
  ...
  handler: async (ctx): Promise<Doc<"users">[]> => {
    // Query for users who don't have firstName or lastName
    const users = await ctx.db
      .query("users")
      .filter((q) => q.or(
        q.eq(q.field("firstName"), undefined),
        q.eq(q.field("lastName"), undefined)
      ))
      .collect();
...
export const getRecentlyActiveUsersWithoutNames = internalQuery({
  args: {},
  handler: async (ctx): Promise<Doc<"users">[]> => {
    // Only look at users active in the last 24 hours (Facebook API requirement)
    ...
    const users = await ctx.db
      ...
      .filter((q) => q.and(
        ...
        q.gte(q.field("lastActiveAt"), twentyFourHoursAgo)
      ))
      .collect();
